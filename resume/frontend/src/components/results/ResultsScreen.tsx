...
export default function ResultsScreen() {
  ...
  const handleToggleStatus = async () => {
    ...
    try {
      await toggleJobStatus(currentRecord.id, !isOpen);
      updateRecordLocally(currentRecord.id, { isOpen: !isOpen });
    } catch (err) {
          ...
          <section className="rounded-[16px] border border-[#ece6f4] bg-white shadow-[0_4px_18px_rgba(111,81,154,0.05)]">
            <div className="flex items-center justify-between border-b border-[#f0ebf6] px-6 py-4">
              <div className="flex items-center gap-3 flex-wrap">
                ...
                {/* Hiring status badge */}
                <span className={`rounded-full px-3 py-1 text-[11px] font-semibold ${isOpen ? 'bg-[#edfaf1] text-[#3aac5d]' : 'bg-[#fff1f1] text-[#de556c]'}`}>
                ...
                <button
                  ...
                  className={`flex items-center gap-1.5 rounded-lg border px-3 py-1.5 text-[12px] font-semibold transition-colors disabled:opacity-50 ${
                    isOpen
                      ? 'border-[#f7aab5] bg-[#fff4f6] text-[#de556c] hover:bg-[#ffedf1]'
                      : 'border-[#c0f0cf] bg-[#edfaf3] text-[#2d7a4f] hover:bg-[#d6f5e2]'
                  }`}
                >
                  {togglingStatus ? (
                    ...
                    <svg width="12" height="12" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="2"><rect x="3" y="3" width="10" height="10" rx="1.5"/></svg>
                  ) : (
                    <svg width="12" height="12" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="2"><path d="M3 8a5 5 0 1 0 10 0 5 5 0 0 0-10 0zm5-2v4m-2-2h4"/></svg>
                  )}
                  {togglingStatus ? 'Updating…' : isOpen ? 'Close Hiring' : 'Reopen Hiring'}
                </button>
              </div>
...
//   const [stageFilter, setStageFilter] = useState('');
//   const [sortBy, setSortBy] = useState('score');
//   const [isOpen, setIsOpen] = useState<boolean>(currentRecord?.isOpen !== false);
//   const [togglingStatus, setTogglingStatus] = useState(false);
...
//       await toggleJobStatus(currentRecord.id, !isOpen);
//       setIsOpen(!isOpen);
//     } catch (_) { /* silent — user will see no change */ }
...
//                 </div>
//                 {/* Hiring status badge */}
//                 <span className={`rounded-full px-3 py-1 text-[11px] font-semibold ${isOpen ? 'bg-[#edfaf1] text-[#3aac5d]' : 'bg-[#f5f0fa] text-[#9c90af]'}`}>
//                   {isOpen ? '● Hiring' : '○ Closed'}
//                 </span>
//                 {/* Toggle button */}
//                 <button
