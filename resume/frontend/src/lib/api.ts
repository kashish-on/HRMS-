...
// Attaches the Supabase JWT to every request so the backend can verify the
...
// Used exclusively for the public /careers/* endpoints that job applicants hit.
...
// ── HR-only API functions (all use authenticated request()) ───────────────────
...
// ── Public Careers Portal API (no auth — intentionally open) ─────────────────
...
export async function fetchPublicJobs(): Promise<PublicJob[]> {
  const payload = await requestPublic<{ jobs?: PublicJob[] }>('/careers/jobs');
  return payload.jobs || [];
}

export async function fetchPublicJob(jobId: string): Promise<PublicJob> {
  const payload = await requestPublic<{ job: PublicJob }>(`/careers/jobs/${jobId}`);
  return payload.job;
}
...
export async function submitPortalApplication(
  ...
  return requestPublic<{ uploadId: string }>(`/careers/jobs/${jobId}/apply`, {
    method: 'POST',
    body: form,
  });
}
