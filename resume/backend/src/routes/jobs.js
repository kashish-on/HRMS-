...
// ── PATCH /jobs/:id/status — open or close hiring ────────────────────────────
router.patch("/:id/status", async (req, res) => {
  try {
    ...
    if (typeof is_open !== "boolean") {
      return res.status(400).json({ status: "error", message: "is_open must be a boolean." });
    }
    ...
    if (error) {
      console.error("[jobs] toggle status error:", error);
    ...
    return res.json({
      status: "ok",
      message: is_open ? "Job reopened successfully." : "Job closed successfully.",
      job: data,
    });
  } catch (err) {
