...
const locationMatchRatio = (jobLocation, candidateLocations) => {
  ...
  const nearbyTargets = getNearbyLocations(target);
  ...
  for (const location of candidateLocations || []) {
    ...
    if (tokenRatio > 0) {
      bestRatio = Math.max(bestRatio, tokenRatio * 0.9);
      continue;
    }
 
    // 4. Proximity match — same metro cluster → 90% credit
    ...
    if (isNearJobLocation) {
      bestRatio = Math.max(bestRatio, 0.9); // Same metro area → 90% credit
      continue;
    }
 
    // 5. Reverse proximity — 90% credit
