...
const CandidateDetail = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (bgData) setBgVerificationData(bgData);
      ...
      if (itAssetData) setItAssetDetails(itAssetData);
      ...
      if (inductionData) setInductionDetails(inductionData);
      ...
      if (probationData) setProbationDetails(probationData);
  ...
  const employeeName = employee?.name?.trim() || "";
  const employeeInitials = employeeName
    ? employeeName.split(" ").filter(Boolean).slice(0, 2).map((part: string) => part[0]?.toUpperCase() || "").join("")
    : "--";
  const employeeDepartment = employee?.departments?.name || employee?.department_name || employee?.department || "--";
  const employeeJoiningDate = employee?.joining_date || employee?.join_date || "--";
  const employeeType = normalizeEmployeeType(employee?.type || employee?.employment_type || "");
  const backgroundCompanyName = bgVerificationData?.company_name?.trim() || "--";
  const backgroundReferenceName = bgVerificationData?.reference_person_name?.trim() || "--";
  const backgroundReferenceEmail = bgVerificationData?.reference_person_email?.trim() || "--";
  const backgroundReferencePhone = bgVerificationData?.reference_person_phone?.trim() || "--";
    ...
    <div className="space-y-6">
      ...
      {isLoading ? (
        ...
        <>
          ...
          <Card>
            <CardContent className="p-6">
              <div className="flex flex-col sm:flex-row items-start sm:items-center gap-5">
                <Avatar className="h-20 w-20 border-2 border-border">
                  <AvatarImage
                    src={employee?.photo_url?.startsWith("http") ? employee.photo_url : undefined}
                  />
                  <AvatarFallback className="bg-muted text-muted-foreground text-xl font-semibold">{employeeInitials}</AvatarFallback>
                </Avatar>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <h2 className="text-xl font-bold text-foreground">{employee?.name || "Candidate"}</h2>
                    <span className="inline-flex items-center rounded-full bg-green-50 px-2.5 py-0.5 text-xs font-medium text-green-700 border border-green-200">
                      In Progress
                    </span>
                  </div>
          ...
          {/* Personal Details — now includes emergency contact, blood group, linkedin */}
          <Card>
            <CardHeader className="pb-4">
              <CardTitle className="text-base font-semibold">Candidate Personal Details</CardTitle>
            </CardHeader>
            <CardContent className="grid gap-4 md:grid-cols-2">
              <div className="space-y-3">
                ...
                <div>
                  ...
                  <p className="text-sm font-medium text-foreground">{employee?.emergency_contact || "--"}</p>
                </div>
              </div>
              ...
              <div className="space-y-3">
                ...
                <div>
                  ...
                  <p className="text-sm font-medium text-foreground">{employee?.blood_group || "--"}</p>
                </div>
              </div>

              {/* ✅ LinkedIn — full width row */}
              <div className="md:col-span-2">
                <p className="text-xs uppercase tracking-wide text-muted-foreground">LinkedIn Profile</p>
              ...
              <div className="flex items-center justify-between px-4">
                {detailOnboardingSteps.map((step, index) => (
                  <div key={index} className="flex items-center flex-1 last:flex-none">
                    ...
                    {index < detailOnboardingSteps.length - 1 && (
                      <div className="flex-1 mx-2">
                        <div className={`h-0.5 w-full ${step.completed ? "bg-green-500" : "bg-muted"}`} />
                      </div>
                    )}
                  </div>
                ))}
                          ...
                          <div className="flex items-center justify-center gap-2">
                              ...
                              <TooltipTrigger asChild>
                                <button
                                  onClick={async () => {
                                    if (!doc.fileUrl) return;
                                    const newWindow = window.open("about:blank", "_blank");
                                    if (!newWindow) { alert("Allow popups for this site."); return; }
                                    try {
                                      const response = await fetch(doc.fileUrl);
                            ...
                            <Tooltip>
                              <TooltipTrigger asChild>
                                <button
                                  onClick={() => handleStatusChange(index, "Approved")}
                                  className={`p-1 rounded ${doc.status === "Approved" ? "bg-green-500 text-white" : "hover:bg-green-100 text-green-600"}`}
                                >
                                  <CheckCircle2 className="h-4 w-4" />
                                </button>
                              </TooltipTrigger>
                            ...
                            <Tooltip>
                              <TooltipTrigger asChild>
                                <button
                                  onClick={() => { setSelectedIndex(index); setRejectModal(true); }}
                                  className={`p-1 rounded ${doc.status === "Rejected" ? "bg-red-500 text-white" : "hover:bg-red-100 text-red-600"}`}
                                >
                                  ✕
                                </button>
                              </TooltipTrigger>
            ...
            <CardContent className="space-y-5">
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  ...
                  <button
                    onClick={() =>
                      saveBgVerificationStatus(bgVerificationStatus === "pending" ? "complete" : "pending")
                    }
                    className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium border mt-1 cursor-pointer transition-colors ${
                      bgVerificationStatus === "complete"
                        ? "bg-green-50 text-green-700 border-green-200"
              ...
              <CardContent className="space-y-5">
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Device Photo Upload</p>
                    {itAssetDetails?.device_photo_url ? (
                      <a href={itAssetDetails.device_photo_url} target="_blank" rel="noreferrer" className="text-sm font-medium text-primary hover:underline">
                        View Uploaded File
                      </a>
                    ) : (
                      <p className="text-sm font-medium text-foreground">--</p>
