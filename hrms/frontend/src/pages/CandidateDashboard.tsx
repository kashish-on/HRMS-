...
const CandidateDashboard = () => {
  ...
  useEffect(() => {
    const getUserData = async () => {
      ...
      if (candidateData) {
        ...
        setIfscCode(candidateData.bank_ifsc || "");
        // ✅ Load saved values
        setEmergencyContact(candidateData.emergency_contact || "");
        setBloodGroup(candidateData.blood_group || "");
  ...
  const handleSubmitToHrVerification = async () => {
    ...
    try {
      ...
      // ✅ Include new fields in candidate update
      const candidatePayload: Record<string, any> = {
        aadhar_number: aadharNumber || null,
        pan_number: panNumber || null,
        bank_name: bankName || null,
    ...
    <CandidateWrapper>
      <div className="flex w-full">
        <div className="flex-1 p-6 space-y-6">
          ...
          {/* ✅ NEW — Personal Info card before Background Verification */}
          <Card>
            <CardHeader>
              <CardTitle className="text-sm">Additional Personal Details</CardTitle>
            </CardHeader>

            <CardContent className="space-y-4">
              <div className="space-y-1.5">
                <p className="text-xs text-muted-foreground">Emergency Contact Number</p>
                <Input
              ...
              <div className="space-y-1.5">
                <p className="text-xs text-muted-foreground">LinkedIn Profile URL</p>
                <Input
